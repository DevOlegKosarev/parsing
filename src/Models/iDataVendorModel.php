...
/**
 * Vendor model for working with cscart_companies table
 */
class iDataVendorModel extends Model
{
    // Specify the name of the vendors table in cscart
    protected $table = 'cscart_companies';

    // Specify the primary key of the table
    protected $primaryKey = 'company_id';

    // Specify the type of returned data
    protected $returnType = 'array';

    /**
     * Get the vendor identifier by its name
     *
     * @param string $name The name of the vendor
     * @return int|null The vendor identifier or null if not found
     * @throws DataException If vendor not found
     */
    public function getVendorIdByName(string $name): ?int
    {
        // Find the vendor by the company field
        $vendor = $this->where('company', trim($name))->first();

        // If the vendor is found, return its identifier
        if ($vendor) {
            return (int) $vendor['company_id'];
        }

        // Otherwise throw an exception with an error message
        throw DataException::forInvalidArgument('Vendor not found: ' . $name);
    }
